...
# Derived from work of 高橋かずひと(https://twitter.com/KzhtTkhs)
...
# Taken from https://qiita.com/yumion/items/6eeb820c1f06839d57a7
...
# Function for sending string messages to server in Unity
...
def main():
    ...
    # Camera preparation ###############################################################
    # Get camera stream from a realsense camera
    ...
    while True:
        ...
        # Camera capture #####################################################
        # Separate color and depth frames from realsense stream
        ret, frames = cap.read()
        image = frames[0]
        depth = frames[1]
        if not ret:
