...
public class KaguyaMove : MonoBehaviour
{
    ...
    float previousAngle;
    float startTime;
    float interactionWaveTime = 5.0f;
    float interactionMoeTime = 3.0f;
    ...
    public GameObject CenterMonitor;
...
/*        MovePositionController();
    ...
    private void InteractionMoe()
    {
        ...
        if (Time.time - startTime < interactionMoeTime && State == InteractionState.Start)
        {
            ...
            if (animator.GetCurrentAnimatorClipInfo(0)[0].clip.name == "JUMP00" && MoeStart == true)
            {
                CenterMonitor.GetComponent<AudioSource>().PlayOneShot(MoeMoe);
                MoeStart = false;
            }
            else if (animator.GetCurrentAnimatorClipInfo(0)[0].clip.name != "JUMP00")
            {
                MoeStart = true;
            }
        }
